...
import ExamModeToggle from './components/Header/ExamModeToggle.jsx';
import ChatBubble from './components/Chat/ChatBubble.jsx';
import SidebarPanel from './components/Sidebar/SidebarPanel.jsx';
import ProgressCard from './components/Gamification/ProgressCard.jsx';
...
export default function AppClean() {
  ...
  useEffect(() => {
    if (activeChatId !== null) writeLocal('g9_active', String(activeChatId));
  }, [activeChatId]);
  ...
  useEffect(() => {
    async function loadGamification() {
      try {
        const res = await apiFetch('/gamification/get_points?email=' + encodeURIComponent('guest@student.com'), { method: 'GET' });
        if (res.ok) {
          const data = await res.json();
      ...
      try {
        const res = await apiFetch('/gamification/get_badges?email=' + encodeURIComponent('guest@student.com'), { method: 'GET' });
        if (res.ok) {
          const data = await res.json();
        ...
        <SidebarPanel activeTab={activeTab} onTabChange={setActiveTab}>
            ...
            <div style={{ padding: '1rem' }}>
              ...
              <div style={{ display: 'grid', gap: '0.5rem' }}>
                {(chats || []).map((c) => (
                  ...
                  >
                    <div style={{ fontWeight: 'inherit' }} className="g9-fw-700">{c.title || 'Chat'}</div>
          ...
          {activeTab === 'settings' ? (
            <div style={{ padding: '1rem', display: 'grid', gap: '0.75rem' }}>
              <div>
                <div className="g9-fw-700" style={{ marginBottom: '0.25rem' }}>Subject</div>
              ...
              <div>
                <div className="g9-fw-700" style={{ marginBottom: '0.25rem' }}>Language</div>
          ...
          {activeTab === 'gamification' ? (
            <div style={{ padding: '1rem', display: 'grid', gap: '0.5rem' }}>
              ...
              <ProgressCard
                title="Next badge"
                value={points < 100 ? '100' : String(Math.ceil(points / 500) * 500)}
